...
logger = logging.getLogger("MonitorDimPDV")

class MonitorDimPDV:
    """
    Production-grade monitoring class for Silver PDV dimension.
    Evaluates data quality metrics, detects trends, and generates actionable alerts.
    """
    ...
    def _write_monitoring_table(self, df: DataFrame):
        ...
        df.write.format("delta").mode("append").partitionBy("execution_date").option("overwriteSchema", "true").saveAsTable(self.monitoring_table)
    ...
    def run(self) -> DataFrame:
        ...
        """
        logger.info("Starting MonitorDimPDV run (serverless-optimized)")
        try:
            ...
            logger.info("MonitorDimPDV run complete")
            return df_monitoring
        except Exception as e:
            logger.error(f"Critical error in MonitorDimPDV: {e}", exc_info=True)
            raise
...
def run_monitoring_dim_pdv(
    ...
    """
    Runs the MonitorDimPDV for orchestration or local testing.
...
if __name__ == "__main__":
    # Local test/demo run
    logger.info("Running MonitorDimPDV in standalone mode")
