"""
Bronze Monitoring Script: sell_in
Author: Diego Mayorga

Purpose:
    Monitor and audit post-ingestion data quality for workspace.bronze.sell_in.
...
def run_monitoring(
    ...
    validation_table: str = "workspace.bronze.sell_in_validation",
    batch_id: str = None,  # Optional: monitor only specific batch
    load_date: str = None, # Optional: monitor by date
    fully_empty_threshold: float = 0.05,
    env: str = "dev"
):
    """
    Perform monitoring and audit of a Bronze Delta table after ingestion.
    Supports scalable monitoring by batch_id or load_date to avoid full-table scans.

    Args:
        bronze_table (str): Bronze Delta table to monitor.
        validation_table (str): Delta table to store validation metrics.
        batch_id (str, optional): Monitor only the specified batch.
        load_date (str, optional): Monitor only the specified load date (YYYY-MM-DD).
    ...
    if batch_id is None:
        latest_batch = spark.sql(f"""
            ...
            FROM {bronze_table}
            GROUP BY _batch_id
            ORDER BY MAX(_ingestion_timestamp) DESC
            LIMIT 1
        """).first()
        if not latest_batch:
            print(f"[{env}] WARNING: Bronze table is empty, no batches to monitor")
            return [], []  # No metrics or alerts
        batch_id = latest_batch["_batch_id"]
    if batch_id:
    ...
    if batch_id:
        ...
        source_file = source_row["_source_file"] if source_row else None
    else:
        source_file = None
